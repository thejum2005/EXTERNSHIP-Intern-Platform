...
def dashboard():
    today = date.today()
    attendance_today = Attendance.query.filter_by(intern_id=current_user.id, date=today).first()
    pending_leaves = Leave.query.filter_by(intern_id=current_user.id, status="Pending").count()
    assigned_projects = Project.query.filter_by(intern_id=current_user.id, status="Assigned").count()
    assigned_tasks = Task.query.filter_by(intern_id=current_user.id, status="Assigned").count()

    intern_friday_latest = (
        InternFriday.query.filter_by(intern_id=current_user.id).order_by(InternFriday.date.desc()).first()
    )
...
@intern_required
def attendance():
    q = Attendance.query.filter_by(intern_id=current_user.id).order_by(Attendance.date.desc(), Attendance.time.desc())
    ...
    today = date.today()
    attendance_today = Attendance.query.filter_by(intern_id=current_user.id, date=today).first()
    return render_template("attendance.html", page=p, attendance_today=attendance_today, today=today)
...
def mark_attendance():
    ...
    # Get status from form (Present or Absent)
...
def leave():
    if request.method == "POST":
        ...
        try:
            d = datetime.strptime(leave_date_str, "%Y-%m-%d").date()
        except ValueError:
            flash("Invalid leave date.", "danger")
            return redirect(url_for("intern.leave"))

        item = Leave(intern_id=current_user.id, leave_date=d, leave_type=leave_type, reason=reason, status="Pending")
        db.session.add(item)
        db.session.commit()
        flash("Leave request submitted.", "success")
        return redirect(url_for("intern.leave"))

    q = Leave.query.filter_by(intern_id=current_user.id).order_by(Leave.leave_date.desc(), Leave.id.desc())
    page = request.args.get("page", 1, type=int)
    per_page = current_app.config.get("ITEMS_PER_PAGE", 10)
    p = paginate(q, page=page, per_page=per_page)
...
@intern_required
def projects():
    q = Project.query.filter_by(intern_id=current_user.id).order_by(Project.deadline.asc(), Project.id.desc())
    page = request.args.get("page", 1, type=int)
    per_page = current_app.config.get("ITEMS_PER_PAGE", 10)
    p = paginate(q, page=page, per_page=per_page)
...
def complete_project(project_id: int):
    ...
    if p.intern_id != current_user.id:
...
def complete_task(task_id: int):
    ...
    if t.intern_id != current_user.id:
        flash("Not allowed.", "danger")
...
@intern_required
def feedback():
    q = Feedback.query.filter_by(intern_id=current_user.id).order_by(Feedback.date.desc(), Feedback.id.desc())
    page = request.args.get("page", 1, type=int)
    per_page = current_app.config.get("ITEMS_PER_PAGE", 10)
    p = paginate(q, page=page, per_page=per_page)
...
@intern_required
def intern_friday():
    q = InternFriday.query.filter_by(intern_id=current_user.id).order_by(InternFriday.date.desc())
    page = request.args.get("page", 1, type=int)
    per_page = current_app.config.get("ITEMS_PER_PAGE", 10)
    p = paginate(q, page=page, per_page=per_page)
